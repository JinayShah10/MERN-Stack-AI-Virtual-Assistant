...
const SignIn = () => {
  ...
  return (
    <div className='w-full min-h-screen bg-cover bg-center flex justify-center items-center px-4 py-8' style={{ backgroundImage: `url(${bg})` }}>
      ...
      <form className='w-full sm:w-[90%] h-auto max-w-150 bg-[#00000090] backdrop-blur shadow-lg shadow-black flex flex-col items-center justify-center gap-4 sm:gap-5 px-5 sm:px-7.5 py-8 sm:py-7.5 rounded-3xl sm:rounded-4xl' onSubmit={handleSignIn}>

        <h1 className='text-white text-[26px] sm:text-[30px] md:text-[36px] text-center font-semibold mb-4 sm:mb-7.5'>Sign In To Your <span className='text-blue-400'>Virtual Assistant</span></h1>
        ...
        <button className='min-w-37.5 h-13 sm:h-15 bg-white rounded-full text-black font-semibold text-[17px] sm:text-[19px] mt-4 sm:mt-7.5 hover:bg-blue-400' disabled={loading} >{loading ? "Loading.." : "Sign In"}</button>

        <p className='text-[white] text-[16px] sm:text-[18px] cursor-pointer text-center' onClick={() => { navigate("/signup") }}>Don't have an account? <span className='text-blue-400'>Sign Up</span></p>
      </form>
    </div>
  )
}
